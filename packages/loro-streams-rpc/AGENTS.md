...
- `session/file-get` is the same envelope for transcript file download. Official
  cloud stays on `/session-files/...`; local and self-hosted have no such HTTP
  API, so the web client walks `offset` until `eof` and assembles a browser
  download. Chunks stay inside the File Preview binary budget (~3 MiB).
